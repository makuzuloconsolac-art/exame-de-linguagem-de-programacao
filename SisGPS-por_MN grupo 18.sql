...
--
-- Host: 127.0.0.1
-- Tempo de geração: 19-Jun-2026 às 18:40
...
--
-- Estrutura da tabela `configuracao_smtp`
--

CREATE TABLE `configuracao_smtp` (
  `id` int(11) NOT NULL,
  `smtp_servidor` varchar(100) NOT NULL,
  `smtp_porta` int(11) NOT NULL DEFAULT 587,
...
(1, 'Ana Lopes', 'ana@empresa.ao', NULL, 2, 1, 0, '2026-06-08 20:53:17'),
...
--
-- Estrutura da tabela `registos_acesso`
--

CREATE TABLE `registos_acesso` (
  `id` int(11) NOT NULL,
  `utilizador_id` int(11) NOT NULL,
  `data_acesso` datetime NOT NULL DEFAULT current_timestamp(),
  `ip_endereco` varchar(50) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

--
-- Extraindo dados da tabela `registos_acesso`
--

INSERT INTO `registos_acesso` (`id`, `utilizador_id`, `data_acesso`, `ip_endereco`) VALUES
(1, 4, '2026-06-19 17:02:27', '127.0.0.1'),
(2, 4, '2026-06-19 17:13:47', '127.0.0.1'),
(3, 2, '2026-06-19 17:16:16', '127.0.0.1'),
(4, 4, '2026-06-19 17:30:06', '127.0.0.1'),
(5, 4, '2026-06-19 17:32:58', '127.0.0.1');
...
(5, 'Novo Sprint', NULL, '2026-06-19', '2026-06-19', 0, 4, '2026-06-19 13:42:47');
...
(15, 'fdddddd', 'ghgg', 1, 1, 2.0, 0.0, NULL, 3, 3, '2026-06-19 15:03:40', '2026-06-19 15:03:40');
...
INSERT INTO `utilizadores` (`id`, `username`, `password_hash`, `nome`, `nivel_acesso`, `membro_id`, `activo`, `created_at`, `ultimo_acesso`, `pergunta_seguranca`, `resposta_seguranca`) VALUES
(1, 'admin', '240be518fabd2724ddb6f04eeb1da5967448d7e831c08c8fa822809f74c720a9', 'Ana Lopes', 0, 1, 1, '2026-06-19 15:55:16', NULL, NULL, NULL),
(2, 'bruno', '956cd9e97c44cbb5c9f18ccf258bb3ea0fbac91d027d2ac9489185f531478242', 'Bruno Ndozi', 1, 2, 1, '2026-06-19 15:55:16', '2026-06-19 17:16:16', NULL, NULL),
(3, 'clara', 'c7b418ece9991f8e5182e257e8e609c8d2a7a49d3d3f140692a52ab75b197030', 'Clara Pinto', 1, 3, 1, '2026-06-19 15:55:16', NULL, NULL, NULL),
(4, 'eu', '240be518fabd2724ddb6f04eeb1da5967448d7e831c08c8fa822809f74c720a9', 'Ana Lopes', 0, 1, 1, '2026-06-19 15:56:23', '2026-06-19 17:32:58', NULL, NULL),
(5, 'Nsukula', 'a665a45920422f9d417e4867efdc4fb8a04a1f3fff1fa07e998e86f7f7a27ae3', 'Bruno Ndozi', 1, 2, 1, '2026-06-19 15:56:23', NULL, NULL, NULL),
(6, 'sola', 'a665a45920422f9d417e4867efdc4fb8a04a1f3fff1fa07e998e86f7f7a27ae3', 'Clara Pinto', 1, 3, 1, '2026-06-19 15:56:23', NULL, NULL, NULL);
...
--
-- Índices para tabela `configuracao_smtp`
--
...
ALTER TABLE `emails`
  ADD PRIMARY KEY (`id`),
  ADD KEY `fk_email_tarefa` (`tarefa_id`);
...
--
-- Índices para tabela `registos_acesso`
--
ALTER TABLE `registos_acesso`
  ADD PRIMARY KEY (`id`),
...
ALTER TABLE `utilizadores`
  ...
  ADD KEY `fk_utilizador_membro` (`membro_id`);
...
--
-- AUTO_INCREMENT de tabela `configuracao_smtp`
--
ALTER TABLE `configuracao_smtp`
...
--
-- AUTO_INCREMENT de tabela `registos_acesso`
--
ALTER TABLE `registos_acesso`
...
--
ALTER TABLE `emails`
  ADD CONSTRAINT `fk_email_tarefa` FOREIGN KEY (`tarefa_id`) REFERENCES `tarefas` (`id`) ON DELETE SET NULL;
...
--
-- Limitadores para a tabela `registos_acesso`
--
ALTER TABLE `registos_acesso`
  ADD CONSTRAINT `registos_acesso_ibfk_1` FOREIGN KEY (`utilizador_id`) REFERENCES `utilizadores` (`id`) ON DELETE CASCADE;
...
--
ALTER TABLE `utilizadores`
  ADD CONSTRAINT `fk_utilizador_membro` FOREIGN KEY (`membro_id`) REFERENCES `membros` (`id`) ON DELETE SET NULL;
COMMIT;
